...
const storedUserPhone = localStorage.getItem('pet-user-phone')?.trim() ?? ''

const onboardingDone = ref(localStorage.getItem('pet-onboarding-done') === '1' && Boolean(storedUserName) && Boolean(storedUserPhone))

const userName = ref(onboardingDone.value ? storedUserName : '')
...
const startOnboardingCountdown = (seconds: number): void => {

  onboardingCountdown.value = Math.max(0, Math.floor(seconds))
  ...
  onboardingTimer = window.setInterval(() => {
    ...
    onboardingCountdown.value = Math.max(0, onboardingCountdown.value - 1)

    if (onboardingCountdown.value === 0 && onboardingTimer) {
...
const logoutUser = (): void => {
  ...
  localStorage.removeItem('pet-user-phone')
...
const requestOnboardingCode = async (): Promise<void> => {

  const normalizedName = onboardingName.value.trim()

  const normalizedPhone = onboardingPhone.value.trim()

  if (!normalizedName || !normalizedPhone || !onboardingAccepted.value) {
  ...
  try {
    ...
    onboardingError.value = err instanceof Error ? err.message : '�� ������� ��������� ���'
...
const getExamIcon = (exam: Exam) => subjectIcons[subjectAliases[exam.subject] ?? exam.subject]
...
const finishExam = async (): Promise<void> => {
  ...
  try {
    ...
    if (hasLocalCorrectAnswers) {
      ...
      examSubmitError.value = '�� ������� ��������� ���������� � API. ������� backend � ����� "���������" ��� ���.'
  ...
  <div class="app-shell">
    ...
    <section v-if="!onboardingDone" class="onboarding-wrap">
      ...
      <article class="card onboarding-card">
        ...
        <label class="username-field">
          ...
          <input v-model="onboardingPhone" type="tel" inputmode="tel" placeholder="+7 999 123-45-67" />
        ...
        <div v-else class="onboarding-actions onboarding-code">
          ...
          <button class="ghost-button" :disabled="onboardingCountdown > 0 || onboardingSending" @click="requestOnboardingCode">
            ...
            {{ onboardingCountdown > 0 ? '��������� ����� ����� ' + onboardingCountdown + '�' : '��������� ��� �����' }}
    ...
    <div v-else>
      ...
      <article v-if="!activeExam" class="card hero">
        ...
        <div class="hero-controls">
          ...
          <div class="hero-links">
            ...
            <button class="ghost-button" @click="logoutUser">�����</button>
      ...
      <nav v-if="!activeExam" class="tabs">
        ...
        <button :class="{ active: tab === 'staff' }" @click="tab = 'staff'">�������</button>

        <button :class="{ active: tab === 'catalog' }" @click="tab = 'catalog'">�����</button>

        <button :class="{ active: tab === 'learning' }" @click="tab = 'learning'">��������</button>

        <button :class="{ active: tab === 'sprint' }" @click="tab = 'sprint'">������</button>

        <button :class="{ active: tab === 'leaderboard' }" @click="tab = 'leaderboard'">���������</button>
